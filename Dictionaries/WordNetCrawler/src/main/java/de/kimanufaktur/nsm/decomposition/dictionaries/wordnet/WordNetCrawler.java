...
public class WordNetCrawler {
  ...
  // String path2Dict = "/usr"+ File.separator + "share" + File.separator + "wordnet"+
  // File.separator + "dict";
  ...
  public static void main(String[] args) {
    ...
    antonyms = wordNetCrawler.getAntonyms(buffer.toString(), worttype, 10);
    Set<IWord> recursiveAntonyms = new HashSet<IWord>();
    recursiveAntonyms.addAll(antonyms);
    for (IWord synonym : antonyms) {
      recursiveAntonyms.addAll(wordNetCrawler.getAntonyms(synonym.getLemma(), worttype, 10));
    }
  ...
  public void initWordNet() {
    ...
    // path2Dict = getClass().getClassLoader().getResource("wordNetDict" +
    ...
    try {
      ...
      e.printStackTrace();
      DictUtil.downloadFileParalell(
          source + dictFileName, path2DBLocation + File.separator + dictFileName);
      DictUtil.unzip(
          path2DBLocation + File.separator + dictFileName, path2DBLocation + File.separator);
  ...
  public Map<String, Set<IWord>> getSensekeyToHypernymsMap(
      String word, POS worttype, int numberfoHypernyms) {
    return getSenseKeyToSynsetMap(word, numberfoHypernyms, worttype, Pointer.HYPERNYM);
  }
  ...
  public Map<String, Set<IWord>> getSensekeyToHyponymsMap(
      String word, POS worttype, int numberfoHypernyms) {
    return getSenseKeyToSynsetMap(word, numberfoHypernyms, worttype, Pointer.HYPONYM);
  }
  /**
   ...
   */
  public Set<IWord> getAntonyms(String word, POS possition, int numberofResults) {
  ...
  public Map<String, Set<IWord>> getSensekeyToAntonymsMap(
      String word, POS possition, int numberofResults) {
    ...
    if (idxWord != null) {
      ...
      for (IWordID wordID : wordIDs) {
        ...
        Set<IWord> antonymsSet = result.get(senseKey);
        if (antonymsSet == null) antonymsSet = new HashSet<>();
        ...
        if (antonyms.size() == 0) {
          // ISynset synset1 = wn.getSynset(wordID.getSynsetID());
          for (IWordID antonym : iword.getRelatedWords(Pointer.ANTONYM)) {
            antonymsSet.add(wn.getWord(antonym));
          }
        }
        // add each antonym id to the result
        List<IWord> words;
        for (ISynsetID sid : antonyms) {
          words = wn.getSynset(sid).getWords();
          antonymsSet.addAll(words);
        }
        result.put(senseKey, antonymsSet);
      }
    }
    return result;
  }
   ...
   * @param synsets the restuling sysnset fitting to the wordform form the first parameter.
   */
  ...
  public void printAntonym(String wordForm, ArrayList<IWord> synsets) {
    ...
    if (!synsets.isEmpty()) {
      ...
      System.out.println(
          "The following antonymes contain '"
              + wordForm
              + "' or a possible base form "
              + "of that text:");
      for (Iterator<IWord> i = synsets.iterator(); i.hasNext(); ) {
        System.out.print(i.next().getLemma());
        if (i.hasNext()) {
      ...
      System.err.println("No antonymes exist that contain " + "the word form '" + wordForm + "'");
    }
  }
   ...
   *
   * @param wordForm the work the synset has been calculated
   * @param synset the restuling hypernyms tree fitting to the wordform form the first parameter
   ...
   *
   * @param wordForm the work the synset has been calculated
   * @param hypernymes the restuling hypernyms tree fitting to the wordform form the first parameter
   */
   ...
   */
  public ArrayList<IWord> getHypernyms(String word, int numberOfMeanings, POS possition) {
   ...
   */
  public Set<IWord> getSynset(String word, int numberOfMeanings, POS possition, Pointer point) {
    Set<IWord> result = new HashSet<IWord>();
  ...
  public Map<String, Set<IWord>> getSenseKeyToSynsetMap(
      String word, int numberOfMeanings, POS possition, Pointer point) {
    Map<String, Set<IWord>> result = new HashMap<>();
    ...
    if (idxWord != null) {
      ...
      for (IWordID wid : wordIDs) {
        IWord iword = wn.getWord(wid);
        String senseKey = iword.getSenseKey().toString();
        Set<IWord> synsetSet = result.get(senseKey);
        if (synsetSet == null) synsetSet = new HashSet<>();

        ISynset synset = iword.getSynset();
        // get the synset
        List<ISynsetID> synsets = synset.getRelatedSynsets(point);

        // add each hypernyms id and synonyms to the result
        List<IWord> words;
        for (ISynsetID sid : synsets) {
          words = wn.getSynset(sid).getWords();
          synsetSet.addAll(words);
        }
        result.put(senseKey, synsetSet);
      }
    }
    return result;
  }
   ...
   * @param numberOfMeanings number of synonyms to get. Because there can be alot. Perhaps we just
   *     wont one?
  ...
  public Map<String, Set<IWord>> getSensekeyToSynonymsMap(
    ...
    if (idxWord != null) {
      ...
      for (IWordID wordid : wordIDs) {
        ...
        String senseKey = iword.getSenseKey().toString();
        Set<IWord> synonymsSet = result.get(senseKey);
        if (synonymsSet == null) synonymsSet = new HashSet<>();
        ...
        result.put(senseKey, synonymsSet);
      }
    }
    return result;
  }
  ...
  public Map<String, IWord> getSenseKeyToWordMap(String word, POS wordType) {
    Map<String, IWord> result = new HashMap<>();
    IIndexWord idxWord = wn.getIndexWord(word, wordType);
    if (idxWord == null) {
      return null;
    } else {
      for (IWordID wordID : idxWord.getWordIDs()) {
        ...
        String senseKey = w.getSenseKey().toString();
  ...
  public Map<String, Set<IWord>> getSensekeyToMeronymsMap(
      String litheral, POS pos, int maxNumberOfMeronyms) {
    Map<String, Set<IWord>> result;
    result = getSenseKeyToSynsetMap(litheral, maxNumberOfMeronyms, pos, Pointer.MERONYM_PART);
    result =
        mergeSensekeyMaps(
            result,
            getSenseKeyToSynsetMap(litheral, maxNumberOfMeronyms, pos, Pointer.MERONYM_SUBSTANCE));
    result =
        mergeSensekeyMaps(
            result,
            getSenseKeyToSynsetMap(litheral, maxNumberOfMeronyms, pos, Pointer.MERONYM_MEMBER));
    return result;
  }

  private Map<String, Set<IWord>> mergeSensekeyMaps(
      Map<String, Set<IWord>> acc, Map<String, Set<IWord>> toMerge) {
    Map<String, Set<IWord>> result = acc;
    for (String senseKey : toMerge.keySet()) {
      Set<IWord> merged = result.get(senseKey);
      if (merged == null) merged = new HashSet<>();
      merged.addAll(toMerge.get(senseKey));
      result.put(senseKey, merged);
    }
    return result;
  }
}
