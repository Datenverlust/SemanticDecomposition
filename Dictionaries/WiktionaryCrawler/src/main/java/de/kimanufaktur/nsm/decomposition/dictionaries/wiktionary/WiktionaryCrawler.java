...
public class WiktionaryCrawler {
  ...
  String path2DBLocation =
      null; // System.getProperty("user.home").toString() + File.separator + ".decomposition" +
  // File.separator + "wiktionary" + File.separator + "en";
  ...
  public synchronized IWiktionaryPage getPage(String word) {
    ...
    // wikisaurusParser.setText(word); //if a word has no senses, the db is extended with senses
    // regarding the wikisaurus.
  ...
  public Map<String, Set<IWiktionaryRelation>> getSenseKeyToRelationsMap(
      String word, RelationType rel) {
    Map<String, Set<IWiktionaryRelation>> result = new HashMap<>();
    if (rel != null) {
      Set<IWiktionaryEntry> entries = getEntries(getPage(word));
      for (IWiktionaryEntry wiktionaryEntry : entries) {
        if (wiktionaryEntry != null) {
          if (wiktionaryEntry.getWordLanguage() != null) {
            for (IWiktionarySense sense : wiktionaryEntry.getSenses()) {
              String senseKey =
                  "WiktionarySense:" + wiktionaryEntry.getId() + ":" + sense.getIndex();
              Set<IWiktionaryRelation> relationSet = new HashSet<>(sense.getRelations(rel));
              result.put(senseKey, relationSet);
            }
          }
        }
      }
   ...
   * @param pos the part of speech to filter the entries. Possible are: NOUN, PROPER_NOUN,
   *     FIRST_NAME, LAST_NAME, TOPONYM, SINGULARE_TANTUM, PLURALE_TANTUM, MEASURE_WORD, VERB,
   *     AUXILIARY_VERB, ADJECTIVE, ADVERB, INTERJECTION, SALUTATION, ONOMATOPOEIA, PHRASE, IDIOM,
   *     COLLOCATION, PROVERB, MNEMONIC, PRONOUN, PERSONAL_PRONOUN, REFLEXIVE_PRONOUN,
   *     POSSESSIVE_PRONOUN, DEMONSTRATIVE_PRONOUN, RELATIVE_PRONOUN, INDEFINITE_PRONOUN,
   *     INTERROGATIVE_PRONOUN, INTERROGATIVE_ADVERB, PARTICLE, ANSWERING_PARTICLE, N
   *     EGATIVE_PARTICLE, COMPARATIVE_PARTICLE, FOCUS_PARTICLE, INTENSIFYING_PARTICLE,
   *     MODAL_PARTICLE, ARTICLE, DETERMINER, ABBREVIATION, ACRONYM, INITIALISM, CONTRACTION,
   *     CONJUNCTION, SUBORDINATOR, PREPOSITION, POSTPOSITION, AFFIX, PREFIX, SUFFIX,
   *     PLACE_NAME_ENDING, LEXEME, CHARACTER, LETTER, NUMBER, NUMERAL, PUNCTUATION_MARK, SYMBOL,
   *     HANZI, KANJI, KATAKANA, HIRAGANA, GISMU, WORD_FORM, PARTICIPLE, TRANSLITERATION
  ...
  public void init() {
    ...
    // wkt = JWKTL.openEdition(new
    // File(getClass().getClassLoader().getResource("dict/DB/").getPath()));
    ...
    if (wkt == null) {
      ...
      try {
        ...
        // e.printStackTrace();
        //                    DictUtil.downloadFile(source + dictFileName,
        // path2DBLocation+dictFileName);

        // the actual unpacked dump file is a file with almost identical name as the downloaded
        // archive - except the archive type ending
        ...
        if (!dumpFile.exists()) {
          ...
          DictUtil.downloadFileParalell(
              source + dictFileName + ".bz2", path2DBLocation + File.separator + dictFileName);
        }
  ...
  protected void initDictDBfromDumpFile() {
    //        dictFile = new
    // File(classLoader.getResource("WiktionaryDict/enwiktionary-20150102-pages-meta-current.xml.bz2").getFile());
    dictFile = new File(path2DBLocation + File.separator + dictFileName);
    // wkt = new WritableBerkeleyDBWiktionaryEdition(new
    // File(getClass().getClassLoader().getResource("WiktionaryDict/DB/").getPath()),false,Runtime.getRuntime().maxMemory() / 10);
    ...
    try {
      ...
      // There seems to be a problem with different german dumps which all lead to a nullpointer
      // exeption
