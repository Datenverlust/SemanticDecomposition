 ...
 * the implementation of the @see
 * de.kimanufaktur.nsm.decomposition.dictionaries.wiktionary.WiktionaryCrawler. The initial call of
 * this function will unpack a wikipedia dump and create a database in the
 * de.kimanufaktur.nsm.decomposition.dictionaries.wiktionary target folder. If this database should
 * be reused, please do not mvn clean before the install. Created by faehndrich on 12.11.14.
 */
public class WiktionaryDictionary extends BaseDictionary {
  ...
  public void init() {
    ...
    if (wiktionaryCrawler == null) {
      if (Config.LANGUAGE.GER
          == Config.LANGUAGE.valueOf(
              Config.getInstance().getUserProps().getProperty(Config.LANGUAGE_KEY))) {

        String path2DB =
            Config.getInstance().getUserProps().getProperty(Config.WIKTIONARY_DB_PATH_GER_KEY);
        String archiveName =
            Config.getInstance().getUserProps().getProperty(Config.WIKTIONARY_DB_GER_ARCHIVE_KEY);
        String sourceURI =
            Config.getInstance()
                .getUserProps()
                .getProperty(Config.WIKTIONARY_DB_ARCHIVE_SOURCE_URI_KEY);
        // System.out.println("Init Wiktionary: DB Path:  " + path2DB +", "+ "Archive Path" +
        // archiveName + " Source Path" + sourceURI);
  ...
  public HashMap<String, Set<Concept>> getSensekeyToSynonymsMap(Concept word) {
    ...
    for (String senseKey : synonymsMap.keySet()) {
      ...
      for (IWiktionaryRelation w : synonymsMap.get(senseKey)) {
        IWiktionaryPage spage = wiktionaryCrawler.getPage(w.getTarget());
        if (spage != null) {
          for (IWiktionaryEntry synonym : spage.getEntries()) {
            Concept s = getConcept(synonym.getPage().getId());
  ...
  private void addSensekeyToSynsetsMap(
      Map<String, Set<IWiktionaryRelation>> synset, Map<String, Set<Concept>> toAdd2) {
    for (String senseKey : synset.keySet()) {
      Set<Concept> conceptSet = new HashSet<>();
      for (IWiktionaryRelation w : synset.get(senseKey)) {
        IWiktionaryPage hpage = wiktionaryCrawler.getPage(w.getTarget());
        if (hpage != null) {
          for (IWiktionaryEntry hyper : hpage.getEntries()) {
            Concept h = getConcept(hyper.getPage().getId());
  ...
  public Map<String, Set<Concept>> getSensekeyToMeronymsMap(Concept word) {
    ...
    for (String senseKey : mer.keySet()) {
      Set<Concept> conceptSet = new HashSet<>();
      for (IWiktionaryRelation w : mer.get(senseKey)) {
        IWiktionaryPage spage = wiktionaryCrawler.getPage(w.getTarget());
        if (spage != null) {
          for (IWiktionaryEntry synonym : spage.getEntries()) {
            Concept m = null;
  ...
  public List<Definition> getDefinitions(Concept word) {
    ...
    if (word.getWordType() == null) {
      ...
      entries =
          wiktionaryCrawler.getEntries(
              wiktionaryCrawler.getPage(word.getLitheral()),
              PartOfSpeech.valueOf(word.getWordType().type()));
    }
  ...
  public Concept getConcept(Object id) {
    ...
    result = Decomposition.createConcept(page.getTitle()); // new Concept(page.getTitle());
    // //Decomposition.createConcept(page.getTitle(),WordType.getType(page.getEntry(0).getWordForms().get(0).getWordForm()));
    //        try {
   ...
   * @return the given concept but the related filled with all word forms known to the wikipedia as
   *     concepts.
   */
  @Override
    ...
    try {
      ...
      for (IWiktionaryEntry w : word.getEntries()) {
        for (IWiktionaryWordForm wordForm : w.getWordForms()) {
          Concept related =
              Decomposition.createConcept(wordForm.getWordForm(), concept.getWordType());
  ...
  /**
   * Get the IWord from the WordNet Database. This is only used to check if a page exists so that we
   * can continue
   *
   ...
   */
  private IWiktionaryPage getWord(String word, WordType wordType)
      throws DictionaryDoesNotContainConceptException {
    ...
    if (page != null) {
      try {
        ...
        result =
            wiktionaryCrawler
                .getEntries(page, wordPOS)
                .iterator()
                .next()
                .getPage(); // TODO: Indeterministically choose here
        // logger.error("there are many concepts of type: " + wordType.type() + " of word: " + word
  ...
  /**
   * Get the IWord from the Wiktionary Database. Here the Wikipedia page is returned withoug
   * pictures or annimations.
   *
   * @param word the literal value we are looking for.
  ...
  private IWiktionaryPage getiWiktionaryPage(String word) {
    ...
    result =
        wiktionaryCrawler.getEntries(wiktionaryCrawler.getPage(word)).iterator().next().getPage();
    return result;
  }
