...
public class Decomposition {
  ...
  //    static Map<Integer, Concept> lockMap = Collections.synchronizedMap(new Hashtable<Integer,
  // Concept>());
  ...
  // private static Map<Integer, Concept> knownConcepts = Collections.synchronizedMap(new
  // HashMap<Integer, Concept>());
  private static ConceptCache conceptCache = ConceptCache.getInstance();
  private static Concept concept = null;
  ExecutorService cachedThreadPool =
      Executors.newFixedThreadPool(
          DecompositionConfig.getThreadCount()); // Executors.newCachedThreadPool();
  ...
  public Decomposition() {
    init();
    concepts2Ignore =
        createConcepts2Ignor(); // Create A list of concepts which should be ignored during
                                // decomposition
    nSMPrimes =
        createPrimes(); // Create list of semantic primes form NSM a leaf nodes for the
                        // decomposition
  ...
  /**
   * Initialize the dictionaries and create list of primes. This includes loading WordNet into
   * memmory, creating the NSM semantic primes and concepts to ignore like "the","an" and "a".
   */
  public static void init() {
    ...
    BaseDictionary wordNetDict =
        WordNetDictionary.getInstance(); // Create WordNet Dictionary in memory
    dictionaries.add(wordNetDict);

    //        BaseDictionary measureMentOntology = new
    // RDFXMLDictionary();//RDFXMLDictionary.getInstance();
...
//            dictionaries.add(wiktionaryDict);
  ...
  /**
   * Create a list of concepts to ignore. Generally this is called stop words. TODO: find a list of
   * stopwords, and load it. Have a reference for it...
   *
   * @return the set of concepts to ignore.
   */
  private static HashSet<Concept> createConcepts2Ignor() {
    // String stopwordFile = System.getProperty("user.home") + File.separator + ".decomposition" +
    // File.separator + "stopwords.txt";
    HashSet<Concept> ignoredConcepts = new HashSet<>(10);

    // List<String> ignores = Arrays.asList("a", "an", "the", "in", "or", "of", "on", "we", "us",
    // "to", "that", "so", "our", "no", "me", "it", "etc", "for", "by", "as", "at", "am");
  ...
  private static HashSet<Concept> createPrimes() {
    ...
    // List<String> primes = Arrays.asList("i", "you", "someone", "something", "thing", "people",
    // "body", "kind", "part", "this", "same", "other", "else ", "one", "two", "much", "many",
    // "some", "all", "good", "bad", "big", "small", "think", "know", "want", "feel", "see", "hear",
    // "say ", "words", "true", "do", "happen", "move", "touch", "there is", "have", "be", "live",
    // "die", "when", "now", "before", "after", "a long time", "a short time", "for some time",
    // "moment", "where", "here", "above", "below", "far", "near", "side", "inside", "not", "maybe",
    // "can", "because", "if", "very", "more", "like", "same");
    // german
    // List<String> primes = Arrays.asList("ich", "du", "jemand", "etwas", "ding", "leute",
    // "körper", "art", "teil", "dieses", "gleiche", "andere", "eins", "zwei", "viele", "einige",
    // "alle", "gut", "schlecht", "groß", "klein", "denken", "wissen", "wollen", "fühlen", "sehen",
    // "hören", "sagen", "wort", "wahr", "tun", "geschehen", "bewegen", "berühren", "es gibt",
    // "haben", "befinden", "sein", "leben", "sterben", "wann", "jetzt", "vorher", "nachher", "lange
    // Zeit", "kurze Zeit", "eine Zeit lang", "moment", "wo", "ort", "hier", "über", "unter",
    // "fern", "nah", "seite", "innerhalb", "nicht", "vielleicht", "können", "weil", "falls",
    // "sehr", "mehr", "wie");
  ...
  /**
   * Create a concept for the given word with the given wordtype. If the wordtype is null, the word
   * type of the created concept will be WordType.UNKNOWN
   *
   * @param word the word we want to turn in to a concept.
   * @param wordType the part of speech (POS) of the given word. This is important since the
   *     selection of the definition used for the decomposition will depend on the word type.
   *     Privide it if you can, so the decomposition will be more precise.
  ...
  /**
   * Decompose the concept using its definitions and the synonyms. if the decomposition depth is
   * reached, the decomposition is set to null. TODO: Here all other decomposition possibilities
   * should be implemented. (Meronyms are still missing)
   *
   * @param concept the concept which is subject to decomposition. The result is found in
   *     concept.decomposition.
   * @param decompositionDepth integer to specify the depth of the decomposition. If set to -1
   *     infinity is used.
   */
  private Concept multiThreadedDecompose(Concept concept, int decompositionDepth) {
    if (0 < decompositionDepth) {
      ...
      if (knownConcept.getDecompositionlevel() < 0
          || knownConcept.getDecompositionlevel() < decompositionDepth) {
        ...
        if (lock == null) {
          ...
          // lock.notifyAll();
          // System.out.println("Notify: " + concept.getLitheral() + " with " + lockcount + " locks
          // in use.");
          // }
  ...
  /**
   * Lock oncept in the lockMap and fill concept with all the dictionaries defined in the init()
   * method.
   *
  ...
  private void DecomposeChildren(Concept concept, int decompositionDepth) {
    ...
    // now lets multiThreadedDecompose all synonyms so that the graph gets broader and we can faster
    ...
    // now lets multiThreadedDecompose all hyponyms so that the graph gets broader and we can faster
    // find common concepts.
    decomposeChildren(concept.getHyponyms(), decompositionDepth);
    // now lets multiThreadedDecompose all hypernyms so that the graph gets broader and we can
    // faster find common concepts.
    decomposeChildren(concept.getHypernyms(), decompositionDepth);
    // now lets multiThreadedDecompose all antonyms so that the graph gets broader and we can faster
    // find common concepts.
    decomposeChildren(concept.getAntonyms(), decompositionDepth);
    // now lets multiThreadedDecompose all meronyms so that the graph gets broader and we can faster
    // find common concepts.
  ...
  private void decomposeChildrenSingleThreaded(
      Set<Concept> concepts2Decompost, int decompositionDepth) {
    for (Concept child : concepts2Decompost) {
      if (child.equals(concept) || lockMap.get(child) != null) {
        continue;
      } else {
  ...
  public Concept decompose(Concept concept, int decompositionDepth) {
    if (0 < decompositionDepth) {
      ...
      if (knownConcept.getDecompositionlevel() < 0
        ...
        Concept lock = lockMap.get(concept.hashCode());
        if (lock == null) {
          // System.out.println("locking: " + concept.getLitheral() + " with " + lockcount + " locks
          // in use.");
          lockAndFillConcept(concept, decompositionDepth);
  ...
  /**
   * Fill a definitions of a concept manually. This is recursive, which is not perfect. TODO: change
   * the recursion.
   *
  ...
  */
  /**
   * Decompose the concept which are connceted to the given concept in an single threaded
   * environment.
   *
  ...
  /**
   * Decompose the concept which make up the definition of the given concept in an single threaded
   * environment.
   *
   * @param concept the concept to decompose the children for.
   * @param decompositionDepth the decomposition depth to decompose the children with
   ...
   */
  private Future<Concept> startDecompositionThread(
      Concept concept2decompose, int decompositionDepth) {

    Future<Concept> future =
        cachedThreadPool.submit(
            new Callable<Concept>() {
              @Override
              public Concept call() {
                // System.out.println("Execute Call for concept: " +
                // concept2decompose.getLitheral());
  ...
  /**
   * In an concurrent implementation of the decomposition it is necessarry to wait on decompositions
   * which finish concepts we want to use.
   *
  ...
  private void waitOnLock(Concept concept, Object lock) {
    synchronized (lock) {
      try {
        lockcount++;
        // System.out.println("Wait: " + concept.getLitheral() + " with " + lockcount + " locks in
        // use.");
        lock.wait();
        lockcount--;
        // System.out.println("Continuing: " + concept.getLitheral() + " with " + lockcount + "
        // locks in use.");
      } catch (InterruptedException e) {
        e.printStackTrace();
      }
  ...
  public String toString() {
    ...
    if (concept.getDefinitions() != null) {
      stringBuilder
          .append(" " + "Definition: ")
          .append(concept.getDefinitions().toString())
          .append(System.getProperty("line.separator"));
    } else {
      stringBuilder
          .append(" " + "Definition: No Definition")
          .append(System.getProperty("line.separator"));
    }
    if (concept.getDecomposition() != null) {
      stringBuilder
          .append(" " + "Decomposition: ")
          .append(concept.getDecomposition().toString())
          .append(System.getProperty("line.separator"));
      ...
      stringBuilder
          .append(" " + "Decomposition: No Decomposition ")
          .append(System.getProperty("line.separator"));
    }
    stringBuilder
        .append("==============================")
  ...
  /**
   * Print the decomposition in the terminal. This has been done to evaluate the results of the
