...
package de.kimanufaktur.nsm.decomposition.graph.conceptCache;
...
import de.kimanufaktur.nsm.graph.entities.relations.Synonym;
...
/**
 * This is a utility class providing all needed functionality to create semantic networks. This
 * class has an own decomposition and can be used to multiThreadedDecompose concepts and transform
 * them inte graphs. Created by faehndrich on 30.04.15.
 */
public class GraphUtil {
  ...
  private static HashMap<Integer, DefaultListenableGraph<Object, Object>> mergedGraphCache =
      new HashMap<>(65);
  ...
  public static DefaultListenableGraph<Concept, WeightedEdge> createJGraph(Concept concept) {
    ...
    DefaultListenableGraph<Concept, WeightedEdge> graph =
        new DefaultListenableGraph(new DefaultDirectedWeightedGraph(WeightedEdge.class));
    assert (Decomposition.getConcepts2Ignore().size() > 0);
    if (Decomposition.getConcepts2Ignore().contains(concept)) {
      // System.out.println("The concept " + concept.getLitheral() + " is an stop word and is not
      // decomposed.");
    } else {
  ...
  public static Graph<Entity, Relation> createJGraph(Entity entity) {
    ...
    SemanticNet graph =
        (SemanticNet)
            new DefaultListenableGraph(new DefaultDirectedWeightedGraph<>(Relation.class));
  ...
  /**
   * Add all nodes relevant to the graph of the given context to the graph. Caution only definitions
   * and synonyms are added here. TODO: We want this graph to be structured with multiple node and
   * link types. TODO: We want the links/edges to have a wight of the semantic distance.
   *
   * @param graph the jGraph to add the nodes to
   * @param entity the entity to be added to the graph. TODO: this is incomplete.
   */
  private static void addEntityRecursivly(SemanticNet graph, Entity entity) {
    if (graph.containsVertex(entity)) {
      entity.getRelations().stream()
          ...
          .forEach(
              relation -> {
                relation.getRoles().stream()
                    .filter(role -> !graph.containsEdge(role))
                    .forEach(
                        role -> {
                          graph.addEdge(role.getSource(), role.getTarget(), role);
                        });
                if (graph.containsVertex(relation)) {
                  if (!graph.containsEdge(entity, relation)) graph.addEdge(entity, relation);
                } else {
   ...
   */
  private static void addConceptRecursivly(
      DefaultListenableGraph<Concept, WeightedEdge> graph, Concept concept) {
    if (!Decomposition.getConcepts2Ignore().contains(concept)) {
      if (!graph.containsVertex(concept)) {
        graph.addVertex(concept);
        // add weight as semantic distance or as relational similarity. 0 meaning the same word and
        // 1 meaning with a maximal distance
        concept.getSynonyms().stream()
            .filter(
                syn ->
                    syn != null
                        && !Decomposition.getConcepts2Ignore().contains(syn)
                        && !syn.equals(concept))
            .forEach(
                syn -> {
                  ...
                  synonymEdge.setAttributes(
                      attributes); // add weight as semantic distance or as relational similarity. 0
                                   // meaning the same word and 1 meaning with a maximal distance
        ...
        for (Definition definition : concept.getDefinitions()) {
          ...
          definition.getDefinition().stream()
              .filter(
                  def ->
                      def != null
                          && !Decomposition.getConcepts2Ignore().contains(def)
                          && !def.equals(concept))
              .forEach(
                  def -> { // Take out stop words, so the actiavtion does not screw up.
        ...
        concept.getHyponyms().stream()
            .filter(
                hypo ->
                    hypo != null
                        && !Decomposition.getConcepts2Ignore().contains(hypo)
                        && !hypo.equals(concept))
            .forEach(
                hypo -> {
        ...
        concept.getHypernyms().stream()
            .filter(
                hyper ->
                    hyper != null
                        && !Decomposition.getConcepts2Ignore().contains(hyper)
                        && !hyper.equals(concept))
            .forEach(
                hyper -> {
        ...
        concept.getAntonyms().stream()
            .filter(
                antonym ->
                    antonym != null
                        && !Decomposition.getConcepts2Ignore().contains(antonym)
                        && !antonym.equals(concept))
            .forEach(
                antonym -> {
        ...
        concept.getMeronyms().stream()
            .filter(
                meronym ->
                    meronym != null
                        && !Decomposition.getConcepts2Ignore().contains(meronym)
                        && !meronym.equals(concept))
            .forEach(
                meronym -> {
        ...
        concept.getArbitraryRelations().stream()
            .filter(
                arbitraryRelation ->
                    arbitraryRelation != null
                        && !Decomposition.getConcepts2Ignore().contains(arbitraryRelation)
                        && !arbitraryRelation.equals(concept))
            .forEach(
                arbitraryRelation -> {
  ...
  /**
   * Add all nodes relevant to the graph in BDOS. Caution only hypernyms and meronyms are added
   * here. TODO: We want this graph to be structured with multiple node and link types. TODO: We
   * want the links/edges to have a wight of the semantic distance.
   *
   * @param graph the jGraph to add the nodes to
   * @param concept the concept to be added to the graph.
   */
  private static void addConceptRecursivlyforBDOS(
      DefaultListenableGraph<Concept, WeightedEdge> graph, Concept concept) {

    BaseDictionary wordnet = WordNetDictionary.getInstance();
    ArrayList<Concept> meronyms = new ArrayList<>();
    for (Set<Concept> meronymSet : wordnet.getMeronyms(concept).values()) {
      meronyms.addAll(meronymSet);
    }
   ...
   * @throws IOException
   */
  public static void saveToGraphML(final Graph<Concept, WeightedEdge> graph, String filename)
      throws IOException {
    logger.info("Saving decomposition to " + filename);
    // GraphMLExporter<String, MyWeightedEdge> exporter = new GraphMLExporter<concept,
    // DefaultEdge>();

    // In order to be able to export edge and node labels and IDs, we must implement providers for
    // them
    ComponentNameProvider<Concept> vertexIDProvider =
        vertex -> {
    ...
    ComponentNameProvider<Concept> vertexNameProvider =
        vertex -> {
          if (vertex != null) {
            return vertex.getLitheral();
          } else {
    ...
    ComponentNameProvider<WeightedEdge> edgeIDProvider =
        edge -> {
          ...
          if (edge instanceof ArbitraryEdge) {
            edgeType =
                graph.getEdgeSource(edge)
                    + " "
                    + ((ArbitraryEdge) edge).getRelationName()
                    + " > "
    ...
    ComponentNameProvider<WeightedEdge> edgeLabelProvider =
        edge -> {
          if (edge != null) {
    ...
    ComponentAttributeProvider<WeightedEdge> edgeAttributeProvider =
        edge -> {
    ...
    exporter.registerAttribute(
        "edgeType", GraphMLExporter.AttributeCategory.EDGE, AttributeType.STRING);
    //        exporter.registerAttribute("weight", GraphMLExporter.AttributeCategory.EDGE,
  ...
  /**
   * Save the given concept to the given graph. The result is stored unter
   * ~/.decomposition/CONCEPTNAME_DEPTH.decompGraph or .GraphML
   *
   * @param jGraph the JGraphT to save the concepts decomposition in
   * @param concept the concept to add to the given graph
   * @param decompositionDepth the decomposition decompositionDepth, which is only used for the name
   *     of the file.
   */
   ...
   * @throws IOException
   */
  public static void saveGraphObject(Graph DecompositionjGraph, String path2write2)
      throws IOException {
  ...
  /**
   * Load the given graph form the user home directory in .decomposition with the ednign
   * .decompGraph
   *
   ...
   */
  public static Graph loadGraph(String decompositionWord, WordType wordType, int decompositionDepth)
  ...
  /**
   * Create an entity out of a concept. This needs to be done when transformation to the semantic
   * networks and the formal model of concept of Mahr is used. Mahr, B. (2007). Ein Modell der
   * Auffassung. Technische Universitaet Berlin.
   *
   * @param c the concept to be transformed into a entity
   * @return the entity representing the given concept. TODO: This is incomplete since not only an
   *     entity but all its relations and the related entities need to be added.
   */
    ...
    while (iterator.hasNext()) {
      ...
      if (sourceEdge instanceof WeightedEdge) {
        if (!result.containsEdge(sourceEdge)) {
          result.addEdge(
              ((WeightedEdge) sourceEdge).getSource(),
              ((WeightedEdge) sourceEdge).getTarget(),
              sourceEdge);
        }
      }
    }
  ...
  public static Graph getGraph(
      String decompositionWord, WordType wordType, int decompositionDepth) {
    Graph graph = null;
    if (MarkerPassingConfig.isUseGraphCache()) {
      ...
      // Concept c = decomposition.multiThreadedDecompose(decompositionWord, wordType,
      // decompositionDepth);
      Concept c = decomposition.decompose(decompositionWord, wordType, decompositionDepth);
  ...
  /**
   * Merging two graphs into one. Taking all node and edges from the first one and adding those of
   * the second graph if they do not already exist.
   *
   * @param graph1 the first graph to merge
   * @param graph2 the second graph to merge into the first one
   * @return a new graph containing all edges and nodes of the first two graphs.
   */
